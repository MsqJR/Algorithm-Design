...
int main()
{
    ...
    while(num_nodes!=0)
    {
        ...
        double* mini = new double[num_nodes];
        for (int i = 0; i < num_nodes; i++)
        {
            visited[i] = false;
            mini[i] = 9999999;
        ...
        for (int i = 0; i < num_nodes; i++)
        {
            ...
            for (int j = 0; j < num_nodes; j++) {
                if (!visited[j] && (u == -1 || mini[j] < mini[u]))
                {
                    u = j;
                }
            }
            ...
            {
                Edge edge = g.nodes[u].adj[k];
                if (!visited[edge.v] && edge.w < mini[edge.v])
                {
                    mini[edge.v] = edge.w;
                }
            }
        ...
        cout << "Frog Distance = "<< setprecision(3)<< fixed<< MAX_Dist<< endl<< endl;
